class Reports::DriversOverallSpeed
	def self.info
		return {:name => 'Drivers Overall Statistics', :key => 'driversOverallSpeed'} 
	end

	def self.generate(user, date_from, date_to, params)
		columns = ['First Name', 'Last Name', 'Max speed', 'Avg speed', 'Avg. RPM', 
			'Avg. Fuel', 'Dongle Pts Coll.', 'Avg. Beh. Pts', 'Total Beh. Points']
		values = []
		...
		users.each do |u|
			trips = Trip.where('user_id = ? AND stats_gen = TRUE AND trip_stats.dongle_points > 0', u.id).eager_load(:trip_stat)
